// Automatically created by 'sqlite auto migrator (SAM)' on 2024-05-02T22:10:02.411Z
...
export async function up(db) {
    await db.run("CREATE TABLE temp_l3k6rpgabp(event_id INTEGER NOT NULL,business_id INTEGER NOT NULL,user_id TEXT NOT NULL,timestamp TEXT NOT NULL,status TEXT NOT NULL,PRIMARY KEY(event_id,user_id)ON CONFLICT REPLACE,FOREIGN KEY(business_id)REFERENCES Businesses(id),FOREIGN KEY(event_id)REFERENCES Events(id)ON DELETE CASCADE,FOREIGN KEY(user_id)REFERENCES Users(id))");
    await db.run("INSERT INTO temp_l3k6rpgabp (\"event_id\", \"business_id\", \"user_id\", \"timestamp\", \"status\") SELECT \"event_id\", \"business_id\", \"user_id\", \"timestamp\", \"status\" FROM \"records\"");
    await db.run("DROP TABLE \"records\"");
    await db.run("ALTER TABLE temp_l3k6rpgabp RENAME TO \"records\"");
}
