...
// import test utils
const { describe, it, after, before } = require('node:test'); // read about the builtin Node.js test framework here: https://nodejs.org/docs/latest-v18.x/api/test.html
...
describe('Client', () => {
    ...
    function supportsGoogleSignIn() {
        return (
            ...
            [undefined, 'chrome'].includes(process.env.SELENIUM_BROWSER)
        );
    }
    ...
    describe('Authentication', () => {
        it(
            'Should signin and redirect correctly via dev login',
            { skip: supportsGoogleSignIn() },
            async () => {
                await driver.get(
                    `http://localhost:${port}/login.html?redirect=http://localhost:${port}`,
                );
        ...
        it(
            'Should signin and redirect correctly via prod login',
            { skip: !supportsGoogleSignIn() },
            async () => {
        ...
        it('Signout should work', async () => {
            ...
            try {
                ...
                assert.strictEqual(
                    JSON.stringify(e),
                    '{"name":"TimeoutError","remoteStacktrace":""}',
                );
            }
        });
    });
